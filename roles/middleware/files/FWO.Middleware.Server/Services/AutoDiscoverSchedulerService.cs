...
namespace FWO.Middleware.Server.Services
{
    ...
    /// </summary>
    public class AutoDiscoverSchedulerService : IAsyncDisposable
    {
        ...
        public AutoDiscoverSchedulerService(ISchedulerFactory schedulerFactory, ApiConnection apiConnection, GlobalConfig globalConfig, IHostApplicationLifetime appLifetime)
        {
        ...
        private async void OnStarted()
        {
            try
            {
                ...
                configSubscription = apiConnection.GetSubscription<List<ConfigItem>>(ApiExceptionHandler, OnGlobalConfigChange, ConfigQueries.subscribeAutodiscoveryConfigChanges);
        ...
        private async Task ScheduleJob()
        {
            ...
            DateTimeOffset startTime = CalculateStartTime(globalConfig.AutoDiscoverStartAt, TimeSpan.FromHours(globalConfig.AutoDiscoverSleepTime));
            ...
            ITrigger trigger = TriggerBuilder.Create()
                ...
                .WithSimpleSchedule(x => x
                    .WithInterval(TimeSpan.FromHours(globalConfig.AutoDiscoverSleepTime))
                    .RepeatForever())
                .Build();

            await scheduler.ScheduleJob(trigger);

            Log.WriteInfo(SchedulerName, $"Trigger scheduled. Start: {startTime:yyyy-MM-dd HH:mm:ss}, Interval: {globalConfig.AutoDiscoverSleepTime}h");
        }
